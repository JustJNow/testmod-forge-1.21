package net.justjnow.testmod.item.custom;
...
public class ModBowItem extends ProjectileWeaponItem {
    ...
    private int range;
    private int drawSpeed; // in ticks

    public ModBowItem(Item.Properties properties, float damageMultiplier, int range, int drawSpeed) {
        super(properties);
    ...
    public void releaseUsing(ItemStack pStack, Level pLevel, LivingEntity pEntityLiving, int pTimeLeft) {
        if (pEntityLiving instanceof Player player) {
            ...
            if (!itemstack.isEmpty()) {
                ...
                float f = getPowerForTime(i, this.drawSpeed);
                if (!((double)f < 0.1)) {
                    List<ItemStack> list = draw(pStack, itemstack, player);
                    if (pLevel instanceof ServerLevel serverlevel && !list.isEmpty()) {
                        this.shoot(serverlevel, player, player.getUsedItemHand(), pStack, list, f * 3.0F * this.damageMultiplier, 1.0F, f == 1.0F, null);
                    }
    ...
    public static float getPowerForTime(int pCharge, int drawSpeed) {
        float f = (float)pCharge / (float)drawSpeed;
        f = (f * f + f * 2.0F) / 3.0F;
        if (f > 1.0F) {
            f = 1.0F;
        }
